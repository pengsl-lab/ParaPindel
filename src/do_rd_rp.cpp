   ...
   for (unsigned SampleIndex = 0; SampleIndex < AllSamples.size(); SampleIndex++) {
      ...
      for (unsigned ChrIndex = 0; ChrIndex < ChrNameSizes.size(); ChrIndex++) {
         ...
         for (unsigned ScanIndex = 0; ScanIndex < NumberOfScan; ScanIndex++) {
            ...
            for (unsigned posIndex = 0; posIndex < BinSize / Skip + 1; posIndex++) {
               CoverageCount[posIndex] = 0;
            }
            ...
            binStart += BinSize;
            binEnd += BinSize;
            if (binStart > ExtraRegionSize) {
               binStart_Extra = binStart - ExtraRegionSize;
            } else {
               binStart_Extra = 1;
            }
            if (binEnd + ExtraRegionSize < ChrNameSizes[ChrIndex].ChrSize) {
               binEnd_Extra = binEnd + ExtraRegionSize;
            } else {
               binEnd_Extra = ChrNameSizes[ChrIndex].ChrSize;
            }
         }
         // report RP
      }
