                                       ...
                                       const Chromosome* chromosome, std::vector<SPLIT_READ>& split_reads)
{
   ...
   for (size_t i = 0; i < bam_sources.size(); i++) {
      ...
      openAndReadInBamReads_SR(source.bamFileName.c_str(), chromosome->getName(), &chromosome->getSeq(), split_reads,
                                 ...
                                 const SearchWindow& window, UserDefinedSettings* userSettings)
{
   ...
   for (size_t i = 0; i < bam_sources.size(); i++) {
      ...
      LOG_DEBUG(*logStream << time_log() << "Loading discordant reads from " << source.bamFileName << std::endl);

      // Setup link to bamfile, its index and header.
      samFile* fp = sam_open(source.bamFileName.c_str(), "r");
      hts_idx_t *idx = sam_index_load(fp, source.bamFileName.c_str());

      if (idx == NULL) {
         LOG_WARN(*logStream << time_log() << "Failed to load index for " << source.bamFileName << std::endl);
         LOG_WARN(*logStream << "Skipping window: " << chr_name << ", " << window.getStart() << "--" <<
                  window.getEnd() << " for BAM-file: " << source.bamFileName << std::endl);
         continue;
      }
...
std::map<int, std::string> get_sequence_name_dictionary(ControlState& state)
{
   ...
   for (bam_info_iter = state.bams_to_parse.begin(); bam_info_iter != state.bams_to_parse.end(); ++bam_info_iter) {
      samFile* fp = sam_open((*bam_info_iter).bamFileName.c_str(), "r");
      bam_hdr_t *header = sam_hdr_read(fp);
