   ...
   while (inf_ReadSeq.HasNext()) {
      ...
      if (Temp_One_Read.FragName == FragName && Temp_One_Read.MatchedRelPos >= currentWindow.getStart()
         ...
         if (BufferReads.size () >= NumberOfReadsPerBuffer) {
            ...
            {
               #pragma omp for   // openMP 2.5 requires signed loop index
...
bool ReadInBamCrossReads_RP_Discovery (const char *bam_path,
                                  ...
                                  const SearchWindow& currentWindow,
								  unsigned int current_global_start,
								  unsigned int current_global_end)
{
   ...
   int start = currentWindow.getStart(), end = currentWindow.getEnd();
   if(currentWindow.getEnd()==current_global_end){
	   start = currentWindow.getStart()+1;
	   end = currentWindow.getEnd();
   }else if(currentWindow.getStart() == current_global_start && currentWindow.getEnd()- CROSS_WINDOW_SIZE >0){
	   start = currentWindow.getStart();
	   end = currentWindow.getEnd()- CROSS_WINDOW_SIZE;
   } else if(currentWindow.getEnd()- CROSS_WINDOW_SIZE >0){
	   start = currentWindow.getStart()+1;
	   end = currentWindow.getEnd()- CROSS_WINDOW_SIZE;
   }
...
bool ReadInBamCrossReads_SR (const char *bam_path, const std::string & FragName,
                        const std::string * CurrentChrSeq,
                        std::vector < SPLIT_READ > &LeftReads,
                        std::vector < SPLIT_READ > &OneEndMappedReads,
                        std::vector <REF_READ> &RefSupportingReads,
                        ...
                        bool verbose)
{
   ...
   int start = window.getStart(), end = window.getEnd();
      if(window.getEnd()==global_end){
   	   start = window.getStart()+1;
   	   end = window.getEnd();
      }else if(window.getStart() == global_start && window.getEnd()- CROSS_WINDOW_SIZE >0){
   	   start = window.getStart();
   	   end = window.getEnd()- CROSS_WINDOW_SIZE;
      } else if(window.getEnd()- CROSS_WINDOW_SIZE >0){
   	   start = window.getStart()+1;
   	   end = window.getEnd()- CROSS_WINDOW_SIZE;
      }
   hts_itr_t *iter = sam_itr_queryi (idx, tid, start, end);
...
bool openAndReadInBamReads_SR (const char *bam_path,
						const std::string & FragName,	//CurrentChr Name
                        const std::string * CurrentChrSeq,  // CurrentChrSeq
                        std::vector < SPLIT_READ > &LeftReads,
                        std::vector < SPLIT_READ > &OneEndMappedReads,
                        std::vector <REF_READ> &RefSupportingReads,
                        int InsertSize,
   ...
   if (userSettings->bamFilesAsInput()) {
      ...
      for (unsigned int i = 0; i < currentState.bams_to_parse.size(); i++) {
         ...
         ReturnFromReadingReads = ReadInBamReads_RP(
                                     currentState.bams_to_parse[i].bamFileName.c_str(),
         ...
         if (ReturnFromReadingReads == 0) {
            LOG_ERROR(*logStream << "Bam read failed: " << currentState.bams_to_parse[i].bamFileName << std::endl);
            return 1;
         } else if (currentState.Reads_RP.size() == 0) {
         } else {
            std::cout << currentState.bams_to_parse[i].bamFileName << " RP " << currentState.Reads_RP[i].size() << std::endl;
         }
      }
   }
   return 0;
...
short get_RP_Cross_Reads_Discovery(ControlState& currentState, const SearchWindow& currentWindow, unsigned int current_global_start,unsigned int current_global_end )
{
   short ReturnFromReadingReads;
   RPVector TempOneRPVector;
   ...
   if (userSettings->bamFilesAsInput()) {
      ...
      for (unsigned int i = 0; i < currentState.bams_to_parse.size(); i++) {
         ...
         ReturnFromReadingReads = ReadInBamCrossReads_RP_Discovery(
                                     currentState.bams_to_parse[i].bamFileName.c_str(),
                                     currentWindow.getChromosome()->getName(),
                                     &currentWindow.getChromosome()->getSeq(),
                                     //&currentState.CurrentChrSeq,
                                     ...
                                     currentState.bams_to_parse[i].insertSize,
                                     currentState.bams_to_parse[i].tag,
                                     currentWindow, current_global_start, current_global_end);
         if (ReturnFromReadingReads == 0) {
            *logStream << "Bamfile " << currentState.bams_to_parse[i].bamFileName << " has some kind of error. Aborting Pindel\n";
            return 1;
         }
         if (currentState.Reads_RP_Discovery.size() == 0) {
            *logStream << "No discordant RP reads in Bamfile " << currentState.bams_to_parse[i].bamFileName.c_str() << std::endl;
         } else {
            std::cout << currentState.bams_to_parse[i].bamFileName << " RP " << currentState.Reads_RP_Discovery.size() << std::endl;
         }
      }
   }
   return 0;
      ...
      for (unsigned int i = 0; i < currentState.bams_to_parse.size(); i++) {
         *logStream << "\nInsertsize in config: " << currentState.bams_to_parse[i].insertSize << std::endl;
         ...
         if (ReturnFromReadingReads == 0) { // perhaps 'false'? ReadInBamReads returns a boolean...
            ...
            LOG_ERROR(*logStream << "No currentState.Reads for " << currentWindow.getChromosome()->getName() << " found in " << currentState.bams_to_parse[i].bamFileName << std::endl);
         }
         *logStream << "BAM file index\t" << i << "\nBam file name\t" << currentState.bams_to_parse[i].bamFileName.c_str() << "\nNumber of split-reads so far\t" << currentState.Reads_SR.size() << "\n" << std::endl;
      }
   }
...
short get_SR_Cross_Reads(ControlState& currentState, const SearchWindow& currentWindow, unsigned int gloabl_start, unsigned int global_end )
{
   // std::cout << "in get_SR_Reads " << std::endl;
   ...
   if (userSettings->bamFilesAsInput()) {
      ...
      for (unsigned int i = 0; i < currentState.bams_to_parse.size(); i++) {
         *logStream << "\nInsertsize in config: " << currentState.bams_to_parse[i].insertSize << std::endl;
         //std::cout << "before ReadInBamReads_SR " << std::endl;
         ReturnFromReadingReads = ReadInBamCrossReads_SR(
                                     currentState.bams_to_parse[i].bamFileName.c_str(),
                                     currentWindow.getChromosome()->getName(),
                                     &currentWindow.getChromosome()->getSeq(),
                                     //	&currentState.CurrentChrSeq,
                                     ...
                                     currentState.bams_to_parse[i].insertSize,
                                     currentState.bams_to_parse[i].tag,
                                     currentWindow, readBuffer, gloabl_start, global_end);
         //std::cout << "after ReadInBamReads_SR " << ReturnFromReadingReads << std::endl;
         if (ReturnFromReadingReads == 0) { // perhaps 'false'? ReadInBamReads returns a boolean...
            LOG_ERROR(*logStream << "Bam read failed: " << currentState.bams_to_parse[i].bamFileName << std::endl);
            return EXIT_FAILURE;
...
short openBamAndGet_SR_Reads(ControlState& currentState, const SearchWindow& currentWindow )
{
   // std::cout << "in get_SR_Reads " << std::endl;
   ...
   if (userSettings->bamFilesAsInput()) {
      ...
      for (unsigned int i = 0; i < currentState.bams_to_parse.size(); i++) {
         *logStream << "\nInsertsize in config: " << currentState.bams_to_parse[i].insertSize << std::endl;
         //std::cout << "before ReadInBamReads_SR " << std::endl;
         ReturnFromReadingReads = openAndReadInBamReads_SR(
                                     currentState.bams_to_parse[i].bamFileName.c_str(),
                                     currentWindow.getChromosome()->getName(),
                                     &currentWindow.getChromosome()->getSeq(),
                                     //	&currentState.CurrentChrSeq,
