...
public:
   ...
   int SearchByMpirank(BDData & g_bdData, ControlState& currentState, const unsigned numBoxes, const SearchWindow& window, int mpirank);
protected:
   int Count_Var;
   int Count_Var_Plus;
   int Count_Var_Minus;
   ...
   virtual void outputResultsByMpirank(BDData & g_bdData, ControlState& currentState, std::vector<unsigned> Vars[], const unsigned NumBoxes, const SearchWindow& currentWindow, int mpirank) = 0;
private:
