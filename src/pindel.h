...
extern double time_sortOutputDI, time_sortAndOutputTandemDuplications, time_sortOutputInv;
extern double time_SortOutputInv_NT, time_LargeInsertionOutf, time_sortOutputSI;
int cabs(int a);
...
extern unsigned int CROSS_WINDOW_SIZE;
...
struct bam_info {
   std::string bamFileName; // EW: change to FileName?
   int insertSize;
   std::string tag;
   bam_info()
   {
...
public:
   //LoopingSearchWindow(const SearchRegion* region, const Chromosome* chromosome, const int binSize, const unsigned int start, const unsigned int end, int step);
   ...
   LoopingSearchWindow(const SearchRegion* region, const Chromosome* chromosome, const unsigned int binSize, int global_start, int global_end);
   void next();
   std::string display() const;
   void updateEndPositionsbyStep(int step);
   bool finished() const;
   unsigned int get_chr_start();
   unsigned int get_chr_end();
private:
