...
void ModifyRP(std::vector <RP_READ> & Reads_RP)
{
   ...
   std::cout << "InitializeA1B1 done." << std::endl;
   ...
   #pragma omp parallel default(shared)
   {
      ...
      for (int first = 0; first < (int)Reads_RP.size(); first++) {
         ...
         //std::cout <<Current_first Current_first.ChrNameA << "\t" << Current_first.PosA << "\t" << Current_first.PosB << std::endl;
   ...
   std::cout << "#pragma omp parallel default(shared) done." << std::endl;
