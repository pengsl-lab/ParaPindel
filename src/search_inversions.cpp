...
int searchInversions(ControlState& currentState, unsigned NumBoxes, const SearchWindow& window, int mpirank)
{
   ...
   //std::cout << " Searching inversions ...  -----   currentState.Reads_SR.size() = " << currentState.Reads_SR.size() << std::endl;
   ...
   clock_t start_sortOutputInv = clock();
   SortOutputInv(currentState, NumBoxes, window.getChromosome()->getSeq(), currentState.Reads_SR, Inv, InversionOutf);
   clock_t end_sortOutputInv= 0;
   time_sortOutputInv += (end_sortOutputInv - start_sortOutputInv)/ CLOCKS_PER_SEC;
