...
int searchIndels(ControlState& currentState, unsigned NumBoxes, const SearchWindow& window, int mpirank )
{
      ...
      if (currentRead.MatchedD == Plus) {
         if (currentRead.UP_Far[FarIndex].Direction == Minus) {
			...
			if (currentRead.UP_Far[FarIndex].Direction == Plus) {
				if (currentRead.UP_Close[CloseIndex].LengthStr + currentRead.UP_Far[FarIndex].LengthStr < currentRead.getReadLength() && currentRead.UP_Close[CloseIndex].LengthStr + currentRead.UP_Far[FarIndex].LengthStr >= userSettings->Min_Num_Matched_Bases && currentRead.UP_Close[CloseIndex].AbsLoc > currentRead.UP_Far[FarIndex].AbsLoc + 1) {
   ...
   std::ofstream DeletionOutf( userSettings->getDOutputFilename(mpirank).c_str(), std::ios::app);
   std::ofstream inversionsOutf( userSettings->getINVOutputFilename(mpirank).c_str(), std::ios::app);
