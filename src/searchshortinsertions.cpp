...
void SearchShortInsertions::outputResultsByMpirank(BDData & g_bdData, ControlState& currentState,
      std::vector<unsigned> Vars[], const unsigned NumBoxes, const SearchWindow& currentWindow, int mpirank)
{
   std::ofstream SIoutputfile(userSettings->getSIOutputFilename(mpirank).c_str(), std::ios::app);
   clock_t start_sortOutputSI = clock();
   SortOutputSI(currentState, NumBoxes, currentWindow.getChromosome()->getSeq(), currentState.Reads_SR, Vars, SIoutputfile);
   clock_t end_sortOutputSI = clock();
   time_sortOutputSI += (double)(end_sortOutputSI - start_sortOutputSI)/CLOCKS_PER_SEC;
   SIoutputfile.close();
}
