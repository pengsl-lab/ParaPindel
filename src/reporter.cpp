                ...
                const unsigned int &RealEnd, std::ofstream & TDOutf)
{
   ...
   TDOutf << NumberOfTDInstances << "\tTD " << TDs[C_S].IndelSize	// << " bases "
          << "\tNT " << TDs[C_S].NT_size << " \"" << TDs[C_S].NT_str << "\"" << "\tChrID " << TDs[C_S].FragName
		  << "\tBP " << TDs[C_S].BPLeft << "\t" << TDs[C_S].BPRight + 2 << "\tBP_range "
		  << TDs[C_S].BPLeft << "\t" << TDs[C_S].BPRight + 2
		  << "\tSupports " << NumberOfReads << "\t" << Num_U_Reads << "\t+ "
		  << LeftS << "\t" << LeftUNum << "\t- " << RightS << "\t" << RightUNum << "\tS1 " << EasyScore;	//EWL070111 << "\tS2 " << PreciseScore;
   unsigned int window_size = (unsigned int) (1000000 * userSettings->FLOAT_WINDOW_SIZE);
   if((int)(TDs[C_S].BPLeft / window_size) != (int)((TDs[C_S].BPRight + 2)/ window_size)){
	   std::cout <<
		          "####################################################################################################   "
		          << NumberOfTDInstances << "\tTD " << TDs[C_S].IndelSize	// << " bases "
		          << "\tNT " << TDs[C_S].NT_size << " \"" << TDs[C_S].NT_str << "\"" << "\tChrID " << TDs[C_S].FragName
				  << "\tBP " << TDs[C_S].BPLeft << "\t" << TDs[C_S].BPRight + 2 << "\tBP_range "
				  << TDs[C_S].BPLeft << "\t" << TDs[C_S].BPRight + 2
				  << "\tSupports " << NumberOfReads << "\t" << Num_U_Reads << "\t+ "
				  << LeftS << "\t" << LeftUNum << "\t- " << RightS << "\t" << RightUNum << "\tS1 " << EasyScore;
   }
   int SUM_MS = 0;
...
//DeletionOutf << "EWL BREAKING JENKINS! (see if it still works)\n";
   ...
   //std::cout << "calculateSupportPerStrand finfish!" << std::endl;
   ...
   //std::cout << "entering reportBreakDancerEvent!" << std::endl;
   ...
   DeletionOutf << deletionFileData.getSvIndex() << "\tD " << Deletions[C_S].IndelSize	// << " bases "
				...
				<< "\"" << "\tChrID " << Deletions[C_S].FragName << "\tBP "
				<< Deletions[C_S].BPLeft + 1 << "\t" << Deletions[C_S].BPRight + 1
				<< "\tBP_range " << RealStart + 1 << "\t" << RealEnd + 1 << "\tSupports "
				<< NumberOfReads << "\t" << Num_U_Reads << "\t+ " << LeftS << "\t"
				<< LeftUNum << "\t- " << RightS << "\t" << RightUNum << "\tS1 " << EasyScore;	//EWL070111  << "\tS2 " << PreciseScore;
   LOG_DEBUG(*logStream << "d_6" << std::endl);
   unsigned int window_size = (unsigned int) (1000000 * userSettings->FLOAT_WINDOW_SIZE);
   if((int)(Deletions[C_S].BPLeft + 1)/ window_size != (int)(Deletions[C_S].BPRight + 1 )/ window_size){
	   std::cout <<  "####################################################################################################   "
                << deletionFileData.getSvIndex() << "\tD " << Deletions[C_S].IndelSize	// << " bases "
               	 << "\tNT " << Deletions[C_S].NT_size << " \"" << Deletions[C_S].NT_str
				<< "\"" << "\tChrID " << Deletions[C_S].FragName << "\tBP "
				<< Deletions[C_S].BPLeft + 1 << "\t" << Deletions[C_S].BPRight + 1
				<< "\tBP_range " << RealStart + 1 << "\t" << RealEnd + 1 << "\tSupports "
				<< NumberOfReads << "\t" << Num_U_Reads << "\t+ " << LeftS << "\t"
				<< LeftUNum << "\t- " << RightS << "\t" << RightUNum << "\tS1 " << EasyScore << std::endl;
   }
   int SUM_MS = 0;
   for (unsigned int i = C_S; i <= C_E; i++) {
                       ...
                       const unsigned int &RealEnd, std::ofstream & InvOutf)
{
   ...
   InvOutf << g_numberOfInvInstances++ << "\tINV " << Inv[C_S].IndelSize	// << " bases "
           << "\tNT " << LeftNT_size << ":" << RightNT_size << " \""
		   << LeftNT_str << "\":\"" << RightNT_str << "\""
		   << "\tChrID " << Inv[C_S].FragName
		   << "\tBP " << Inv[C_S].BPLeft + 1 - 1 << "\t" << Inv[C_S].BPRight + 1 + 1
		   << "\tBP_range " << Inv[C_S].BPLeft + 1 - 1 << "\t"
		   << Inv[C_S].BPRight + 1 + 1 << "\tSupports " << NumberOfReads << "\t"
		   << Num_U_Reads << "\t+ " << LeftS << "\t" << LeftUNum << "\t- " << RightS << "\t" << RightUNum << "\tS1 " << EasyScore;	//EWL070111  << "\tS2 " << PreciseScore;
   unsigned int window_size = (unsigned int) (1000000 * userSettings->FLOAT_WINDOW_SIZE);
   if((int)(Inv[C_S].BPRight + 1 + 1 )/window_size  != (int)(Inv[C_S].BPLeft + 1 - 1)/window_size){
	   std::cout <<
	             "####################################################################################################  "
	             << g_numberOfInvInstances++ << "\tINV " << Inv[C_S].IndelSize	// << " bases "
	           << "\tNT " << LeftNT_size << ":" << RightNT_size << " \""
			   << LeftNT_str << "\":\"" << RightNT_str << "\""
			   << "\tChrID " << Inv[C_S].FragName
			   << "\tBP " << Inv[C_S].BPLeft + 1 - 1 << "\t" << Inv[C_S].BPRight + 1 + 1
			   << "\tBP_range " << Inv[C_S].BPLeft + 1 - 1 << "\t"
			   << Inv[C_S].BPRight + 1 + 1 << "\tSupports " << NumberOfReads << "\t"
			   << Num_U_Reads << "\t+ " << LeftS << "\t" << LeftUNum << "\t- " << RightS << "\t" << RightUNum << "\tS1 " << EasyScore;
   }
   int SUM_MS = 0;
   for (unsigned int i = C_S; i <= C_E; i++) {
      SUM_MS += Inv[i].MS;
...
void oddEventsortReads(const std::vector < SPLIT_READ >& Reads, std::vector < unsigned >& VariantIndices)
{
   unsigned int SIsNum = VariantIndices.size();
   unsigned int phase, i;
   UserDefinedSettings* userSettings = UserDefinedSettings::Instance();
#pragma omp parallel num_threads(userSettings->numThreads) \
	default(none) shared(Reads, SIsNum, VariantIndices) private(phase, i)
   for (phase = 0; phase < SIsNum; phase++) {
	   if(phase % 2 == 0)
#pragma omp for
      for (i = 1; i < SIsNum; i+=2) {
         if (!smaller(Reads[VariantIndices[i-1]], Reads[VariantIndices[i]])) {
            std::swap( VariantIndices[i-1],VariantIndices[i]);
         }
      }
	   else
#pragma omp for
      for (i = 1; i < SIsNum -1; i+=2) {
         if (!smaller(Reads[VariantIndices[i]], Reads[VariantIndices[i+1]])) {
            std::swap( VariantIndices[i],VariantIndices[i+1]);
         }
      }
   }
}
                   ...
                   std::ofstream & SIsOutf)
{
   ...
   for (unsigned Box_index = 0; Box_index < NumBoxes; Box_index++) {
      ...
      if (SIs[Box_index].size () >= userSettings->NumRead2ReportCutOff) {
         ...
         oddEventsortReads( Reads, SIs[ Box_index ] );
                                      ...
                                      std::ofstream & TDOutf, const bool nonTemplate)
{
   ...
   for (unsigned Box_index = 0; Box_index < NumBoxes; Box_index++) {
      if (TDs[Box_index].size () >= userSettings->NumRead2ReportCutOff) {
         ...
         oddEventsortReads( AllReads, TDs[ Box_index ] );
...
bool RP_support_D(ControlState& currentState, Indel4output & OneIndelEvent, unsigned start, unsigned end)
{
   ...
   for (unsigned index = 0; index < currentState.Reads_RP_Discovery.size(); index++) {// DA блг╜ DB ChrNameA == ChrNameB == CurrentChrName PosA < start PosB > end
   ...
   for (unsigned Box_index = 0; Box_index < NumBoxes; Box_index++) {
      ...
      if (Deletions[Box_index].size () >= userSettings->NumRead2ReportCutOff) {
        ...
        // std::cout << "S3" << std::endl;
         oddEventsortReads( Reads, Deletions[ Box_index ] );
                  ...
                  */
                  ...
                  if (IndelEvents[EventIndex].Support >= userSettings->NumRead2ReportCutOff && IsGoodDeletion(GoodIndels, IndelEvents[EventIndex], RealStart, RealEnd, currentState)) {
                     ...
                     if (GoodIndels[IndelEvents[EventIndex].Start].IndelSize < userSettings->BalanceCutoff) {
                        ...
                        //std::cout << CurrentChr <<"---" << IndelEvents[EventIndex].Start << "---" << IndelEvents[EventIndex].End <<"---" << RealStart <<"---"<< RealEnd << std::endl;
